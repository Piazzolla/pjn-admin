...
@Component({
    selector: 'dependencia-abm',
    templateUrl: './abm-dependencia.component.html',
    styleUrls: ['./abm-dependencia.component.scss']
})

export class AbmDependenciaComponent {
  data: any; 
  id: string;
  ...
  ngOnInit(){
    ...
    this.dataService.httpFunction(this.dataService.URL_DEPENDENCIA_ID,this,"",parameters);
    this.createForm();
  }
  ...
  responseOk(httpOperation:string, http: string, data:any, ws:any ){
    //Procesar-Data
    switch(ws.name){
      case this.dataService.URL_DEPENDENCIA_ID: 
        this.loaded =true;
