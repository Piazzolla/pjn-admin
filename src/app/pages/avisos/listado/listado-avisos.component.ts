...
export class ListadoAvisosComponent {
  ...
  edit(row){
    let url:string = '/aviso/edit/' + row.id;
  ...
  setPage(pageInfo){
    ...
    this.dataService.httpFunction(this.dataService.URL_AVISO_ALL,this,body,"");
  }
  ...
  responseOk(httpOperation:string, http: string, data:any, ws:any ){
    //Procesar-Data
    switch(ws.name){
      case this.dataService.URL_AVISO_ALL: 
        this.loaded =true;
        this.page = data;
